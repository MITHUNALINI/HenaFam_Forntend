...
class ViewOrder extends Component {
    ...
    handleChangePage = (event, newPage) => {
      ...
      OrderService.getAllOrderInPage(newPage,this.state.rowsPerPage)
      .then((Response) => {
        this.setState({
    ...
    handleChangeRowsPerPage = (event) => {
      ...
      OrderService.getAllOrderInPage(this.state.page,event.target.value)
      .then((Response) => {
        this.setState({
          FeedbackArr:Response.data.data,
          count:Response.data.TotalNoOfElements
    ...
    reloadFeedbackList = () => {
      OrderService.getAllOrderInPage(this.state.page,this.state.rowsPerPage)
         .then((Response) => {
           this.setState({
            FeedbackArr:Response.data.data,
             count:Response.data.TotalNoOfElements
    ...
    deleteById = (PoultryId) => {
      ...
      OrderService.deleteById(PoultryId)
           .then(res => {
               this.setState({
               ...
               this.setState({
                 FeedbackArr: this.state.FeedbackArr.filter(FeedbackArr => FeedbackArr.poultryId !== PoultryId),
                 snackbaropen:true,
                });
           })
        ...
        <Alert onClose={this.snackbarclose} severity="error" variant="filled" style={{position:"fixed",right:"100px",width:'550px',zIndex:3,color:"black"}}>
          {this.state.message}
               ...
               <Paper style = {style.paper}>
                   <h1 style={{fontFamily: 'Raleway',fontSize:40}}>View Orders</h1>
                  ...
                  <TableBody>
                    {this.state.FeedbackArr.map((row) => (
                      <TableRow>
                        ...
                        <TableCell align="left" onClick= {()=>this.deleteById(row.id)}><IconButton ><DeleteIcon  color="default"  inputProps={{ 'aria-label': 'DeleteIcon with default color' }}/></IconButton></TableCell>
